...
class InstitutionServiceImpl implements InstitutionService {
  ...
  InstitutionServiceImpl(
      IndexSearchService<Institution> indexSearchService,
  ...
  public QueryResult<Institution> search(Optional<String> searchText, int page, int limit) {
    ...
    final QueryResult<Institution> queryResult =
        searchText
            .map(text -> indexSearchService.fullTextSearch(Field.DESCRIPTION, text, page, limit))
            .orElseGet(
                () -> indexSearchService.findAll(page, limit, Entity.INSTITUTION.toString()));
  ...
  @Override
  public QueryResult<Institution> search(
      Optional<String> searchText, String categories, int page, int limit) {
    log.trace("search start");
    log.debug(
        "search searchText = {}, categories = {}, page = {}, limit = {}",
        searchText,
        categories,
        page,
        limit);
    final QueryResult<Institution> queryResult =
        searchText
            .map(
                text ->
                    indexSearchService.fullTextSearch(
                        Field.DESCRIPTION,
                        searchText.orElseThrow(),
                        Field.CATEGORY,
                        categories,
                        page,
                        limit))
  ...
  public Institution findById(String id, Optional<Origin> origin, List<String> categories) {
    ...
    if (origin.map(Origin.INFOCAMERE::equals).orElse(false)) {
      ...
      final Supplier<List<Institution>> institutionsSupplier =
          () -> indexSearchService.findById(Field.ID, id);
      List<Institution> institutions;

      Origin orig = origin.get();
      institutions =
          institutionsSupplier.get().stream()
              .filter(
                  institution ->
                      institution.getOrigin().equals(orig)
                          && (categories.isEmpty()
  ...
  void doInstitutionsIndex() {
    ...
    if (institutions.isEmpty()) {
      log.warn(
          "doInstitutionsIndex: no institutions retrieved from IPA open data, skipping AI Search index update");
      return;
    }
  ...
  private List<Institution> getInstitutions() {
    ...
    try (Reader reader =
      ...
      CsvToBean<Institution> csvToBean =
          new CsvToBeanBuilder<Institution>(reader)
