...
@Slf4j
@Service
public class IpaInstitutionSearchServiceConnectorImpl implements IpaSearchServiceConnector {
  ...
  public IpaInstitutionSearchServiceConnectorImpl(
      AzureSearchRestClient azureSearchRestClient, SearchServiceMapper searchServiceMapper) {
    this.azureSearchRestClient = azureSearchRestClient;
    this.searchServiceMapper = searchServiceMapper;
  }

  @Override
  public SearchServiceStatus index(List<Institution> institutions) {
    List<IpaInstitutionIndex> documents =
        institutions.stream().map(IpaInstitutionIndex::fromInstitution).toList();
  ...
  /**
   * Paginates through the entire IPA institution index and returns a map of id → dataAggiornamento
   * for every stored document. Pagination stops when a page returns fewer documents than PAGE_SIZE.
   */
  @Override
    ...
    while (true) {
      SearchServiceIndexResponse<IpaInstitutionIndex> response =
          azureSearchRestClient.searchIpaInstitutions(
              "*", "id,updateDate", true, PAGE_SIZE, skip, null);
  ...
  public String fetchById(String id) {
    ...
    try {
      SearchServiceIndexResponse<IpaInstitutionIndex> response =
          azureSearchRestClient.searchIpaInstitutions("*", "id,updateDate", false, 1, 0, filter);
      ...
      log.error("fetchById: error fetching institution '{}' from AI Search index", id, e);
      return null;
    }
  }

  @Override
  public IpaInstitutionSearchResult search(
      String search, String filter, Integer top, Integer skip) {
    SearchServiceIndexResponse<IpaInstitutionIndex> response =
