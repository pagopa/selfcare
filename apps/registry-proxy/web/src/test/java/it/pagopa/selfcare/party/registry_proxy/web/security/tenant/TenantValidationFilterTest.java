...
class TenantValidationFilterTest {
  ...
  @Test
  void authenticatedTokenWithoutIssuerClaim_shouldSkipEnforcement() throws Exception {
    // A principal is present but the token carries no `iss`: there is no session issuer to
    // reconcile against, so the filter must not invent one (and must not enforce).
    authenticateAs(null, token(null, "AR"));
    filter.doFilter(request, response, filterChain);
  ...
  /**
   * The issuer is taken from the token payload (see the filter's javadoc), so the principal only
   * needs to signal "this request is authenticated"; the {@code issuer} argument is what the
   * caller embeds in {@code credentials} via {@link #token(String, String)}.
   */
  ...
  private String token(String issuer, String tenantId) throws Exception {
    ...
    if (tenantId != null) {
      claims.put(TenantConstants.TENANT_CLAIM, tenantId);
    }
    String payload =
