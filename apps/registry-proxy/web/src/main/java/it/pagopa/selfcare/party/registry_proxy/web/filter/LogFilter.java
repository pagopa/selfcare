...
public class LogFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    final String tenantId = httpServletRequest.getHeader("X-Tenant-Id");
    ...
    String requestBody =
        this.getContentAsString(
            requestCacheWrapperObject.getContentAsByteArray(), request.getCharacterEncoding());
    String responseBody =
        this.getContentAsString(
            responseCacheWrapperObject.getContentAsByteArray(), response.getCharacterEncoding());
    String queryParams =
        requestCacheWrapperObject.getQueryString() != null
            ? requestCacheWrapperObject.getQueryString()
            : "";
    log.info(
        "Request from URI : {} - tenant: {} - method: {} - timelapse: {}ms - Query params {} - Request body {} - Response body {}",
        httpUri,
        tenantId,
        httpMethod,
        endTime,
        MaskDataUtils.maskInformation(queryParams),
        MaskDataUtils.maskInformation(requestBody),
        MaskDataUtils.maskInformation(responseBody));
