package it.pagopa.selfcare.party.registry_proxy.web.security.tenant;
 ...
 * to read the {@code iss} and {@code tenant_id} claims.
 *
 * <p><b>Issuer source:</b> the issuer is read from the token payload rather than from
 * {@code SelfCareUser#getIssuer()}, because the {@code selc-commons-base} version pinned by this
 * app does not expose that accessor on every supported version. Reading both the issuer and the
 * tenant claim from the same already-verified token also removes any chance of the two disagreeing.
 */
@Component
...
public class TenantValidationFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    Map<String, Object> claims = decodeClaims(authentication);
    String issuer = resolveIssuer(authentication, claims);
    ...
    Optional<TenantId> claimTenant = resolveClaimTenant(claims, issuer);
  ...
  private String resolveIssuer(Authentication authentication, Map<String, Object> claims) {
    if (authentication == null || !(authentication.getPrincipal() instanceof SelfCareUser)) {
      return null;
    }
    Object issuer = claims.get(ISSUER_CLAIM);
  ...
  private Optional<TenantId> resolveClaimTenant(Map<String, Object> claims, String issuer) {
    Object rawClaim = claims.get(TenantConstants.TENANT_CLAIM);
    String claimValue = rawClaim == null ? null : rawClaim.toString();
  ...
  /**
   * Decodes the payload of the already cryptographically verified JWT. Returns an empty map when
   * there is no token or it cannot be parsed, so callers fail closed rather than proceeding with
   * partially resolved tenant information.
   */
  private Map<String, Object> decodeClaims(Authentication authentication) {
    ...
    try {
      ...
      LOGGER.warn("Unable to decode JWT payload while resolving tenant information", e);
      return Map.of();
    }
  }
