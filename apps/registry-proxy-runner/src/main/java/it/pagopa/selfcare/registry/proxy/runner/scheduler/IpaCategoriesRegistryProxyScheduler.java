...
@Slf4j
@ApplicationScoped
public class IpaCategoriesRegistryProxyScheduler {
   ...
   * open data sources.
   */
  @Scheduled(cron = "{scheduler.ipa-index-categories.cron}")
  void feedAiSearchIndex() {
    log.info("Starting scheduled AI Search index update");
    try {
      // IPA indexes
      List<IpaCategory> categories = ipaCategoryOpenDataService.fetch();
