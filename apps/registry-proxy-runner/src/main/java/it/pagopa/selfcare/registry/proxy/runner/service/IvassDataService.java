...
public class IvassDataService {
    ...
    /**
     * Fetches IVASS insurance companies from the ZIP endpoint,
     * extracts the CSV, parses it, and filters by registry/work types.
     */
    public List<IvassInsuranceCompany> fetchInsuranceCompanies() {
        log.info("Fetching IVASS insurance companies...");
        try {
            byte[] zip = ivassRestClient.retrieveInsurancesZip();
            ...
            log.info("Fetched {} IVASS insurance companies ({} after filtering)", companies.size(), filtered.size());
            return filtered;
        } catch (Exception e) {
    ...
    private List<IvassInsuranceCompany> parseCsv(byte[] csv) {
        try (Reader reader = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(csv)))) {
            CsvToBean<IvassInsuranceCompany> csvToBean = new CsvToBeanBuilder<IvassInsuranceCompany>(reader)
    ...
    private List<IvassInsuranceCompany> filterCompanies(List<IvassInsuranceCompany> companies) {
        return companies.stream()
                ...
                .filter(c -> {
                    ...
                    return registryTypesAdmitted.stream()
                            .anyMatch(registerTypePrefix::equals);
                })
                .toList();
    }
