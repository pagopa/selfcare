...
public class WebhookSteps {
  ...
  public void iCreateTheWebhook() {
    response =
        given().contentType("application/json").body(webhookRequest).when().post("/webhooks");
  ...
  public void iDeleteTheWebhook() {
    response = given().when().delete("/webhooks/" + lastWebhookProductId);
  }
