...
public class WebhookNotificationRepository implements ReactivePanacheMongoRepository<WebhookNotification> {
    ...
    /**
     * Thread-safe method to find and lock pending notifications.
     * Uses MongoDB's findAndModify to atomically claim notifications for processing.
     * This prevents multiple containers from processing the same notification.
     * 
     ...
     */
    public Uni<List<WebhookNotification>> findAndLockPendingNotifications(int limit, int lockDurationMinutes) {
        ...
        Document update = new Document("$set", new Document()
            .append("processing", true)
            .append("processingUntil", lockUntil));
