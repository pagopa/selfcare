 ...
 * correctly discovered and enforced for a real, {@code @Authenticated} iam endpoint: it requires
 * the {@code X-Tenant-Id} header, reconciles it against the JWT {@code tenant_id} claim, and
 * applies the hub-spid-login default-to-{@code PNPG} exception (Step_0 SELC-1..3).
 */
@QuarkusTest
@TestHTTPEndpoint(IamController.class)
class TenantValidationFilterTest {

  @InjectMock IamService iamService;
  ...
  void ping_withMatchingHeaderAndClaim_shouldReturnOk() {
    when(iamService.ping()).thenReturn(Uni.createFrom().item("OK"));
