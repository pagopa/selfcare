...
Feature: Update User Group
  ...
  Scenario: Successfully suspend a group with a valid ID
    ...
    When I send a POST request to "/v1/user-groups/{groupId}/suspend" to update group
  ...
  Scenario: Successfully activate a group with a valid ID
    Given [UPDATE] user login with username "j.doe" and password "test"
    And I have a valid group ID to update: "6759f8df78b6af202b222d29"
    When I send a POST request to "/v1/user-groups/{groupId}/activate" to update group
    Then [UPDATE] the response status should be 204
  ...
  Scenario: Successfully delete a group with a valid ID
    Given [UPDATE] user login with username "j.doe" and password "test"
    And I have a valid group ID to update: "6759f8df78b6af202b222d29"
    When I send a DELETE request to "/v1/user-groups/{groupId}" to delete group
    Then [UPDATE] the response status should be 204
  ...
  Scenario: Attempt to update a suspended group
    Given [UPDATE] user login with username "j.doe" and password "test"
    And I have a valid group ID to update: "6759f8df78b6af202b222d2a"
    And I have data to update:
      | name       | description | members                                                                   |
      | Group Name | TestGroup   | 525db33f-967f-4a82-8984-c606225e714a,a1b7c86b-d195-41d8-8291-7c3467abfd30 |
    When I send a PUT request to "/v1/user-groups/{groupId}" to update group
