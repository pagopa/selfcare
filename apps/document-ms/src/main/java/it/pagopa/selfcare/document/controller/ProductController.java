...
public class ProductController {
  ...
  public Uni<Response> createProduct(
      @QueryParam("createdBy") String createdBy, @Valid ProductCreateRequest productCreateRequest) {
    return productService
        .createProduct(productCreateRequest, createdBy)
        .onItem()
        .transform(
            productResponse ->
                Response.status(Response.Status.CREATED).entity(productResponse).build());
  }
  ...
  @Operation(
      ...
      description =
          "Partially updates an existing product identified by its productId. "
  ...
  public Uni<Response> patchProductById(
    ...
    return productService
        ...
        .recoverWithItem(
            t -> {
              log.error(
                  "Unexpected error occurred while while parsing data for {}",
                  sanitizedProductId,
                  t);
        ...
        .recoverWithItem(
            t -> {
              log.error(
                  "Unexpected error occurred while patching product with id {}",
                  sanitizedProductId,
                  t);
