...
public class ContractTemplateStorageImplTest {
  ...
  @BeforeEach
  void setup() {
    final BlobServiceAsyncClient blobServiceAsyncClient =
        Mockito.mock(BlobServiceAsyncClient.class);
    ...
    Mockito.when(blobServiceAsyncClient.getBlobContainerAsyncClient("test-container"))
        .thenReturn(blobContainer);
    Mockito.when(blobContainer.getBlobAsyncClient(Mockito.anyString())).thenReturn(blob);
    contractTemplateStorage =
        new ContractTemplateStorageImpl("test-container", blobServiceAsyncClient);
  }

  @Test
  void upload_ShouldUploadFileSuccessfully() throws URISyntaxException {
    final ContractTemplateFile contractTemplateFile =
        ContractTemplateFile.builder()
            .file(
                new File(
                    getClass().getResource("/request/contract-template-fragment.html").toURI()))
            .data(new byte[0])
            .type(ContractTemplateFileType.HTML)
            .build();
    Mockito.when(blob.uploadFromFile(any(), any(), any(), any(), any(), any()))
        .thenReturn(Mono.empty());
    contractTemplateStorage
        .upload("prod-test", "123", contractTemplateFile)
        .subscribe()
        .withSubscriber(UniAssertSubscriber.create())
        .assertCompleted();
    Mockito.verify(blob, Mockito.times(1)).uploadFromFile(any(), any(), any(), any(), any(), any());
    Mockito.verify(blob, Mockito.never())
        .uploadWithResponse(any(), any(), any(), any(), any(), any());
  }
  ...
  void upload_ShouldUploadBytesSuccessfully() {
    final ContractTemplateFile contractTemplateFile =
        ContractTemplateFile.builder()
            ...
            .type(ContractTemplateFileType.HTML)
            .build();
    Mockito.when(blob.uploadWithResponse(any(), any(), any(), any(), any(), any()))
        .thenReturn(Mono.empty());
    contractTemplateStorage
        .upload("prod-test", "123", contractTemplateFile)
        .subscribe()
    ...
    Mockito.verify(blob, Mockito.times(1))
        .uploadWithResponse(any(), any(), any(), any(), any(), any());
  }
  ...
  void upload_Conflict() {
    ...
    final BlobStorageException blobStorageException =
        new BlobStorageException("Conflict", httpResponse, null);
    Mockito.when(blob.uploadWithResponse(any(), any(), any(), any(), any(), any()))
        .thenReturn(Mono.error(blobStorageException));
    var sub =
        contractTemplateStorage
            .upload("prod-test", "123", contractTemplateFile)
            .subscribe()
  ...
  void upload_Error() {
    ...
    final BlobStorageException blobStorageException =
        new BlobStorageException("Error", httpResponse, null);
    Mockito.when(blob.uploadWithResponse(any(), any(), any(), any(), any(), any()))
        .thenReturn(Mono.error(blobStorageException));
    var sub =
        contractTemplateStorage
  ...
  void download_ShouldDownloadFileSuccessfully() {
    final BlobDownloadContentAsyncResponse response =
        Mockito.mock(BlobDownloadContentAsyncResponse.class);
  ...
  void download_NotFound() {
    ...
    final BlobStorageException blobStorageException =
        new BlobStorageException("Not Found", httpResponse, null);
    Mockito.when(blob.downloadContentWithResponse(any(), any()))
        .thenReturn(Mono.error(blobStorageException));
    var sub =
        contractTemplateStorage
            .download("prod-test", "123", ContractTemplateFileType.HTML)
