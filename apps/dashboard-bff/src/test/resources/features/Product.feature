...
Feature: Product
  ...
  Scenario: Successfully retrieving the product roles for given productId and institutionType
    ...
    And The dashboard response body contains the list "roleMappings" of size 5
    And The dashboard response body contains at path "roleMappings" the following list of objects in any order:
    ...
    And The dashboard response body contains at path "roleMappings.find { it.partyRole == 'OPERATOR' }.productRoles" the following list of objects in any order:
      | code           | label              | description                                                       |
      | operator-psp   | Operatore          | Gestisce l’integrazione tecnologica e/o l'operatività dei servizi |
    And The dashboard response body doesn't contain field "roleMappings.find { it.partyRole == 'OPERATOR' }.multiroleAllowed"
    And The dashboard response body contains at path "roleMappings.find { it.partyRole == 'DELEGATE' }.productRoles" the following list of objects in any order:
      | code           | label              | description                                                       |
      | admin-psp      | Amministratore     | Ha tutti i permessi e gestisce gli utenti                         |
    And The dashboard response body doesn't contain field "roleMappings.find { it.partyRole == 'DELEGATE' }.multiroleAllowed"
    And The dashboard response body contains at path "roleMappings.find { it.partyRole == 'ADMIN_EA' }.productRoles" the following list of objects in any order:
      | code           | label              | description                                                       |
      | admin-psp      | Amministratore     | Ha tutti i permessi e gestisce gli utenti                         |
    And The dashboard response body doesn't contain field "roleMappings.find { it.partyRole == 'ADMIN_EA' }.multiroleAllowed"
    And The dashboard response body contains at path "roleMappings.find { it.partyRole == 'SUB_DELEGATE' }.productRoles" the following list of objects in any order:
      | code           | label              | description                                                             |
      | admin-psp      | Amministratore     | Ha tutti i permessi e gestisce gli utenti. Possono essere al massimo 4. |
    And The dashboard response body doesn't contain field "roleMappings.find { it.partyRole == 'SUB_DELEGATE' }.multiroleAllowed"
    And The dashboard response body contains at path "roleMappings.find { it.partyRole == 'MANAGER' }.productRoles" the following list of objects in any order:
      | code           | label              | description                                                       |
      | admin-psp      | Amministratore     | Stipula il contratto e identifica gli amministratori              |
    And The dashboard response body doesn't contain field "roleMappings.find { it.partyRole == 'MANAGER' }.multiroleAllowed"
    And The dashboard response body contains the list "partnerTechRoleMappings" of size 1
    And The dashboard response body contains at path "partnerTechRoleMappings" the following list of objects in any order:
      | partyRole | selcRole | phasesAdditionAllowed |
      | OPERATOR  | LIMITED  | ["dashboard"]         |
    And The dashboard response body contains at path "partnerTechRoleMappings.find { it.partyRole == 'OPERATOR' }.productRoles" the following list of objects in any order:
      | code                 | label                        | description                                                   |
      | partner-operator-psp | Operatore Partner Tecnologico | Gestisce l'integrazione tecnologica per conto del PSP          |
  ...
  Scenario: Successfully retrieving the product roles for given productId
    ...
    And The dashboard response body contains at path "partnerTechRoleMappings.find { it.partyRole == 'OPERATOR' }.productRoles" the following list of objects in any order:
      | code             | label                        | description                                                    |
      | partner-operator | Operatore Partner Tecnologico | Gestisce l'integrazione tecnologica per conto dell'ente         |
  ...
  Scenario: Successfully retrieving the product roles for given productId for prod-interop
    ...
    When I send a GET request to "/v1/products/{productId}/roles" to retrieve productRoles
    Then the response status should be 200
    And The dashboard response body contains the list "roleMappings" of size 4
    And The dashboard response body contains at path "roleMappings" the following list of objects in any order:
      | partyRole    | selcRole | phasesAdditionAllowed           |
      | OPERATOR     | LIMITED  | ["dashboard"]                   |
      | MANAGER      | ADMIN    | ["onboarding"]                  |
