...
public class AuthSteps {
    ...
    public void removeOtpFlowAfterScenario(Scenario scenario) {
        String otpSessionUid = sharedStepData.getResponse().body().jsonPath().getString("otpSessionUid");
    ...
    public void checkResponseBody(Map<String, String> expectedKeyValues) {
        ...
        Map<String, String> claims = jwtService
                .extractClaimsFromJwtToken(sessionToken)
                .await().indefinitely();
    ...
    public void anOtpFlowShouldBeCreatedWithStatus(String status){
        ...
        OtpFlow otpFlow = OtpFlow.<OtpFlow>find(new Document(uiidField, otpSessionUid))
                .firstResult()
                .await()
                .indefinitely();

        Assertions.assertNotNull(otpFlow, "Failed to find OtpFlow with UUID:"+otpSessionUid);
        Assertions.assertEquals(OtpStatus.valueOf(status), otpFlow.getStatus(), "OtpFlow status is not : "+status);
    }
    ...
    public void anOTPFlowWithUuidAlreadyExistsWithStatus(String uuid, String status, int attempts) {
        String updateBuilder = "{'$set': { 'status': ?1, 'attempts' : ?2, 'updatedAt': ?3, 'expiresAt' : ?4 } }";
        OtpFlow.update(updateBuilder, status, attempts, Date.from(OffsetDateTime.now().toInstant()) , Date.from(OffsetDateTime.now().plusMinutes(5).toInstant()))
    ...
    public void theOTPFlowStatusHasBeenUpdatedTo(String uuid, String status) {
        OtpFlow otpFlow = OtpFlow.<OtpFlow>find(new Document(OtpFlow.Fields.uuid.name(), uuid))
                .firstResult()
                .await()
                .indefinitely();
        ...
        Assertions.assertEquals(OtpStatus.valueOf(status), otpFlow.getStatus(),
                "OtpFlow status is not: " + status);
    }
