...
public class SAMLServiceTest {
  @InjectMock SamlValidator samlValidator;
  ...
  public static class SAMLServiceTestProfile implements QuarkusTestProfile {
    ...
    public Map<String, String> getConfigOverrides() {
      return Map.of(
          "saml.idp.cert", FAKE_IDP_CERT, "saml.time.interval", String.valueOf(FAKE_TIME_INTERVAL));
    }
  }
  ...
  public void testGenerateSessionToken_Success() throws Exception {
    ...
    when(samlValidator.validateSamlResponseAsync(
            FAKE_SAML_RESPONSE, FAKE_IDP_CERT, FAKE_TIME_INTERVAL))
        .thenReturn(Uni.createFrom().item(responseMap));

    UserClaims userClaims = new UserClaims();
    userClaims.setEmail("email@test");
    userClaims.setUid("123");
    when(sessionService.generateSessionTokenInternal(any(UserClaims.class)))
        .thenReturn(Uni.createFrom().item("fake-session-token"));

    org.openapi.quarkus.iam_json.model.UserClaims userClaimsApi =
        new org.openapi.quarkus.iam_json.model.UserClaims();
    userClaimsApi.setEmail(userClaims.getEmail());
    userClaimsApi.setUid(userClaims.getUid());
    when(iamApi.saveIAMUser(any(SaveUserRequest.class), isNull()))
    ...
    verify(samlValidator)
        .validateSamlResponseAsync(FAKE_SAML_RESPONSE, FAKE_IDP_CERT, FAKE_TIME_INTERVAL);
  }
  ...
  public void testGenerateSessionToken_Failure() throws Exception {
    ...
    RuntimeException thrown =
        assertThrows(
            ...
            () -> {
              samlValidator.validateSamlResponseAsync(
                  FAKE_SAML_RESPONSE, FAKE_IDP_CERT, FAKE_TIME_INTERVAL);
            });
  }
  ...
  public void testGenerateSessionToken_Exception() {
    ...
    assertEquals(
        "Validation Error", thrown.getMessage(), "The exception message should be propagated");
