Feature: Institution
  ...
  Scenario: Successfully fetches detailed information about users associated with a specific institution
    ...
    And The response body contains the list "[?(@.taxCode=='PRVTNT80A41H401T')][0].products" of size 2
    And The response body contains at path "[?(@.taxCode=='PRVTNT80A41H401T')][0].products" the following list of objects in any order:
    ...
    And The response body contains the list "[?(@.taxCode=='blbrki80A41H401T')][0].products" of size 1
    And The response body contains at path "[?(@.taxCode=='blbrki80A41H401T')][0].products" the following list of objects in any order:
      | roleId                               | productId    | tokenId                              | status  | productRole              | role         | env  | createdAt                |
  ...
  Scenario: Successfully fetches detailed information about users associated with a specific institution filtered by userId
    Given User login with username "j.doe" and password "test"
    And The following path params:
      | institutionId                           | a1b2c3d4-5678-90ab-cdef-1234567890ab                  |
  ...
  Scenario: Successfully fetches detailed information about users associated with a specific institution filtered by products and roles
    Given User login with username "j.doe" and password "test"
    And The following path params:
      | institutionId                           | a1b2c3d4-5678-90ab-cdef-1234567890ab                  |
    And The following query params:
