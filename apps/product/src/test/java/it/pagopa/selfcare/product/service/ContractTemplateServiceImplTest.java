...
public class ContractTemplateServiceImplTest {
    ...
    @InjectMock
    private ContractTemplateStorage contractTemplateStorage;
    ...
    void upload_shouldReturnOk() throws URISyntaxException {
        ...
        Mockito.when(fileUpload.uploadedFile()).thenReturn(Path.of(getClass().getResource("/request/contract-template-fragment.html").toURI()));

        final ContractTemplateUploadRequest uploadRequest = ContractTemplateUploadRequest.builder()
                .productId("prod-test")
                .name("template-test")
                .file(fileUpload)
                .version("1.0.0")
        ...
        final ContractTemplate contractTemplate = ContractTemplate.builder()
                .id("123")
        ...
        Mockito.when(contractTemplateRepository.countWithFilters(Mockito.any(), Mockito.any(), Mockito.any()))
        ...
        Mockito.when(contractTemplateStorage.getContractTemplatePath(Mockito.any(), Mockito.any(), Mockito.any()))
                .thenReturn("contract-templates/prod-test/123.html");

        final ContractTemplateResponse response = contractTemplateService.upload(uploadRequest)
        ...
        Assertions.assertEquals("contract-templates/prod-test/123.html", response.getContractTemplatePath());
    ...
    void upload_StorageError() throws URISyntaxException {
        ...
        contractTemplateService.upload(uploadRequest).subscribe().withSubscriber(UniAssertSubscriber.create()).assertFailed();
    ...
    void upload_Conflict() throws URISyntaxException {
        ...
        var sub = contractTemplateService.upload(uploadRequest)
                .subscribe().withSubscriber(UniAssertSubscriber.create()).assertFailedWith(ConflictException.class);
    ...
    void download_shouldReturnOk() {
        ...
        contractTemplateService.download("prod-test", "template-test", ContractTemplateFileType.HTML)
    ...
    void list_shouldReturnOk() {
        Mockito.when(contractTemplateRepository.listWithFilters(Mockito.any(), Mockito.any(), Mockito.any()))
                .thenReturn(Uni.createFrom().item(List.of()));
        contractTemplateService.list("prod-test", "template-test", "1.0.0")
                .subscribe().withSubscriber(UniAssertSubscriber.create()).assertCompleted();
        Mockito.verify(contractTemplateRepository, Mockito.times(1))
