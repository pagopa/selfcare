...
class ProductMongoReadinessCheckTest {
    ...
    private static final String CONNECTION_STRING =
            "mongodb://user:pwd@mongo-primary.uat.local:27017,mongo-secondary.uat.local:27017/selcProduct?replicaSet=rs0";
    ...
    void setUp() {
        ...
        check = new ProductMongoReadinessCheck(mongoClient, DATABASE, CONNECTION_STRING);
    }
    ...
    void host_showsPlaceholder_whenConnectionStringIsUnparseable() {
        ...
        ProductMongoReadinessCheck resilientCheck =
                new ProductMongoReadinessCheck(mongoClient, DATABASE, "not-a-valid-connection-string");
        when(database.runCommand(Mockito.any(Document.class)))
                .thenReturn(Uni.createFrom().item(new Document("ok", 1.0)));
