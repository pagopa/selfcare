...
public class ContractTemplateServiceImpl implements ContractTemplateService {
  ...
  static {
    try (InputStream is =
        ContractTemplateServiceImpl.class
            .getClassLoader()
            .getResourceAsStream(BASE_TEMPLATE_FILE)) {
    ...
    return contractTemplateRepository
        ...
        .transformToUni(
            count -> {
              if (count > 0) {
                log.warn(
                    "Contract template already exists (productId = {}, name = {}, version = {})",
                    contractTemplate.getProductId(),
                    contractTemplate.getName(),
                    contractTemplate.getVersion());
                return Uni.createFrom()
                    .failure(
                        new ConflictException("Contract template version already exists", "409"));
              }
              log.info(
                  "Persisting new contract template (productId = {}, name = {}, version = {})",
                  contractTemplate.getProductId(),
                  contractTemplate.getName(),
                  contractTemplate.getVersion());
        ...
        .transformToUni(
            ct ->
                contractTemplateStorage
                    .upload(ct.getProductId(), ct.getId(), contractTemplateFile)
                    .onFailure()
                    .invoke(
                        t ->
                            log.error(
                                "Error uploading file for contract template with id {} -> Rolling back database entry",
                                ct.getId()))
                    .onFailure()
                    .call(
                        t ->
                            contractTemplateRepository
                                ...
                                .invoke(
                                    rt ->
                                        log.error(
                                            "Error rolling back database entry for contract template with id {}",
                                            ct.getId(),
                                            rt)))
                    .onItem()
                    .transform(
                        v ->
                            contractTemplateMapper.toContractTemplateResponse(
                                ct,
  ...
  private ContractTemplateFile buildContractTemplateFile(String productId, FileUpload fileUpload) {
    if (fileUpload.contentType().equals(ContractTemplateFileType.HTML.getContentType())) {
      ...
      throw new InternalException(
          "Unsupported contract template file type: " + fileUpload.contentType(), "500");
    }
  }
  ...
  private ContractTemplateFile buildContractTemplateFileFromHTML(
    ...
    return Optional.ofNullable(HtmlUtils.getCleanHTML(fileUpload.uploadedFile().toFile()))
        .map(
            htmlFragment -> {
              String html =
                  BASE_TEMPLATE_HTML.replace(BASE_TEMPLATE_FRAGMENT_PLACEHOLDER, htmlFragment);
              html =
                  html.replace(
        ...
        .orElseThrow(
            () ->
                new InternalException(
                    "Unable to generate a contract template from the provided file", "500"));
  }
}
