...
public class ProductCdcService {
  ...
  private void initOrderStream() {
    ...
    if (!ConfigUtils.getProfiles().contains("test")) {
      try {
        ...
        if (e.getResponse() != null && e.getResponse().getStatusCode() == 404) {
          log.warn(
              "Table StartAt not found (404), starting from now. This is expected on first run.");
        } else {
          log.error(
              "Error connecting to Table Storage. Code: {}, Message: {}",
    ...
    Bson match =
        Aggregates.match(Filters.in("operationType", asList("update", "replace", "insert")));
    Bson project = Aggregates.project(fields(include("_id", "ns", "documentKey", "fullDocument")));
    List<Bson> pipeline = asList(match, project);

    Multi<ChangeStreamDocument<Product>> publisher =
        dataCollection.watch(pipeline, Product.class, options);
    publisher
        .subscribe()
        .with(
            this::consumerEvent,
            failure -> {
              ...
              constructMapAndTrackEvent(
                  failure.getClass().toString(), "FALSE", ProductConstant.PRODUCT_FAILURE_METRICS);
              Quarkus.asyncExit();
            });
        ...
        .with(
            result -> {
              ...
              constructMapAndTrackEvent(
                  document.getDocumentKey().toJson(),
                  "TRUE",
                  ProductConstant.PRODUCT_SUCCESS_METRICS);
            },
            failure -> {
              log.error(
                  "Product CDC update products having id: {} , message: {}",
                  document.getDocumentKey().toJson(),
                  failure.getMessage());
              constructMapAndTrackEvent(
                  document.getDocumentKey().toJson(),
                  "FALSE",
                  ProductConstant.PRODUCT_FAILURE_METRICS);
            });
  ...
  public Uni<Object> invokeCreationDocument(Product product) {
    return Uni.createFrom()
        ...
        .transform(
            products -> {
              List<it.pagopa.selfcare.product.entity.Product> updateProducts =
                  new ArrayList<>(
                      products.stream()
  ...
  public static Map<String, String> mapPropsForTrackEvent(TrackEventInput trackEventInput) {
    Map<String, String> propertiesMap = new HashMap<>();
    Optional.ofNullable(trackEventInput.getDocumentKey())
        .ifPresent(value -> propertiesMap.put("documentKey", value));
    Optional.ofNullable(trackEventInput.getUserId())
        .ifPresent(value -> propertiesMap.put("userId", value));
    Optional.ofNullable(trackEventInput.getProductId())
        .ifPresent(value -> propertiesMap.put("productId", value));
    Optional.ofNullable(trackEventInput.getInstitutionId())
        .ifPresent(value -> propertiesMap.put("institutionId", value));
    Optional.ofNullable(trackEventInput.getProductRole())
        .ifPresent(value -> propertiesMap.put("productRole", value));
    Optional.ofNullable(trackEventInput.getGroupMembers())
        .ifPresent(value -> propertiesMap.put("groupMembers", String.join(",", value)));
    Optional.ofNullable(trackEventInput.getException())
        .ifPresent(value -> propertiesMap.put("exec", value));
