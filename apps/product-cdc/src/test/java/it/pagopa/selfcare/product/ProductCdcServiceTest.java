...
class ProductCdcServiceTest {
    ...
    void setUp() {
        ...
        // Inject the config property manually since we are instantiating the class
        // directly
    ...
    void invokeCreationDocument_shouldUpdateStorageSuccessfully() {
        ...
        when(azureBlobClient.uploadFilePath(anyString(), any(byte[].class)))
        ...
        // Verify that the list to upload contains both the existing product and the
        // new/updated one
        // We can't easily inspect the byte array here without deserializing, but we
        // verified the flow.
    }
    ...
    void consumerEvent_shouldProcessEventAndTrackSuccess() {
        ...
        when(productMapper.toResource(product))
                .thenReturn(new it.pagopa.selfcare.product.entity.Product());
        ...
        Assertions.assertEquals(
                "{\"token\": \"123\"}", captor.getValue().getProperties().get("startAt"));
    ...
    void convertListToJsonBytes_shouldSerializeParentRelationshipFields() {
        // given
        it.pagopa.selfcare.product.entity.Product child = new it.pagopa.selfcare.product.entity.Product();
        child.setId("prod-xy");
