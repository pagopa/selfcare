 ...
 * their {@code TenantValidationFilter} rejects with 400 any request that carries a JWT but no {@code
 * X-Tenant-Id} header. Every internal call made through this factory would therefore fail, so the
 * tenant is propagated alongside the token.
 *
 ...
 * path.
 *
 * <p>When no tenant is resolvable - a call made outside an active request, such as an orchestration
 * callback - the header is simply omitted, mirroring how the Authorization header degrades. Adding
 * a guessed tenant would be worse than sending none.
 */
@ApplicationScoped
