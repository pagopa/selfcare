...
/**
 * Helper che raccoglie le utility di query, recupero e aggiornamento degli onboarding nel database
 * MongoDB.
 */
@Slf4j
@ApplicationScoped
public class OnboardingQueryHelper {
        ...
        .transformToUni(
            opt ->
                opt.map(Onboarding.class::cast)
                    .filter(
                        o ->
                            OnboardingStatus.TOBEVALIDATED.equals(o.getStatus())
  ...
  public Uni<OnboardingGetResponse> constructOnboardingGetResponse(
  ...
  public io.quarkus.mongodb.panache.reactive.ReactivePanacheQuery<Onboarding> runQuery(
  ...
  public Uni<Long> updateReasonForRejectAndUpdateStatus(
      String onboardingId, ReasonRequest reasonForReject) {
              ...
              if (count == 0) {
                return Uni.createFrom()
                    .failure(
                        new InvalidRequestException(
                            String.format(
                                ONBOARDING_NOT_FOUND_OR_ALREADY_DELETED.getMessage(), id)));
              }
