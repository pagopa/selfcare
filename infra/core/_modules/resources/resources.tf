  ...
  provisioner "local-exec" {
    command = <<EOT
            az storage blob sync \
                ...
                --source "${path.module}/../../resources/templates" \
  ...
  provisioner "local-exec" {
    command = <<EOT
            az storage blob sync \
                ...
                --account-name ${replace(replace(var.checkout_cdn_name, "-cdn-endpoint", "-sa"), "-", "")} \
                --account-key ${var.checkout_cdn_storage_primary_access_key} \
                --source "${path.module}/../../resources/products" \
                --destination 'resources/products/' 
              az afd endpoint purge \
                  --resource-group ${var.checkout_fe_rg_name} \
                  --endpoint-name ${replace(var.checkout_endpoint_name, "-afd", "-fde")}  \
                  --profile-name ${replace(var.checkout_endpoint_name, "-cdn-endpoint", "-cdn-profile")}  \
...
resource "null_resource" "upload_resources_default_product_logo" {
  ...
  provisioner "local-exec" {
    command = <<EOT
              ...
              az afd endpoint purge \
                --content-paths "/resources/products/default/logo.png" \
                --resource-group ${var.checkout_fe_rg_name} \
                --endpoint-name ${replace(var.checkout_endpoint_name, "-afd", "-fde")}  \
                --profile-name ${replace(var.checkout_endpoint_name, "-cdn-endpoint", "-cdn-profile")}  \
                --no-wait
  ...
  provisioner "local-exec" {
    command = <<EOT
            az storage blob upload \
              ...
              az afd endpoint purge \
                --content-paths "/resources/products/default/depict-image.jpeg" \
                --resource-group ${var.checkout_fe_rg_name} \
                --endpoint-name ${replace(var.checkout_endpoint_name, "-afd", "-fde")}  \
...
resource "null_resource" "upload_metadata" {
  triggers = {
    file_sha1 = filesha1("${path.module}/../../${var.env}-${var.app_domain}/spid/metadata.xml")
  }
  ...
  provisioner "local-exec" {
    command = <<EOT
            az storage blob upload \
                ...
                --account-name ${replace(replace(var.checkout_cdn_name, "-cdn-endpoint", "-sa"), "-", "")} \
                --account-key ${var.checkout_cdn_storage_primary_access_key} \
                --file "${path.module}/../../${var.env}-${var.app_domain}/spid/metadata.xml" \
                --overwrite true \
...
#   triggers = {
#     dir_sha1 = sha1(join("", [for f in fileset("${path.module}/../../resources/contracts_template/contracts/template", "**") : filesha1("${path.module}/../../resources/contracts_template/contracts/template/${f}")]))
#   }
#   provisioner "local-exec" {
#     command = <<EOT
#               az storage copy \
...
#                 --source "${path.module}/../../resources/contracts_template/contracts/template/*" \
#                 --recursive
#           EOT
#   }
# }
