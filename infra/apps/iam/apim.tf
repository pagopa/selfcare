locals {
  ...
  api_name     = var.is_pnpg ? format("selc-%s-pnpg-api-iam", var.env_short) : format("selc-%s-api-iam", var.env_short)
  display_name = var.is_pnpg ? "PNPG IAM API" : "IAM API"
  base_path    = var.is_pnpg ? "imprese/iam" : "iam"
}


resource "azurerm_api_management_api_version_set" "apim_api_iam" {
  name                = local.api_name
...
module "apim_api_iam_ms" {
  source              = "github.com/pagopa/terraform-azurerm-v4.git//api_management_api?ref=v7.26.5"
  name                = local.api_name
  api_management_name = local.apim_name
  resource_group_name = local.apim_rg
  version_set_id      = azurerm_api_management_api_version_set.apim_api_iam.id
  ...
  content_value = templatefile("../../../apps/iam/src/main/docs/openapi.json", {
