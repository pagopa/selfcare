...
resource "null_resource" "upload_jwks" {
  ...
  provisioner "local-exec" {
    command = <<EOT
              mkdir -p "${path.root}/.terraform/tmp"
              ...
              az storage blob download \
                ...
                --file "${path.root}/.terraform/tmp/oldjwks.json" \
                --name '.well-known/jwks.json'
              python "${path.module}/utils/py/jwksFromPems.py" "${path.root}/.terraform/tmp/oldjwks.json" "${module.jwt.jwt_kid}" "${module.jwt.certificate_data_pem}" "${module.jwt_exchange.jwt_kid}" "${module.jwt_exchange.certificate_data_pem}" > "${path.root}/.terraform/tmp/jwks.json"
              if [ $? -eq 1 ]
              then
                exit 1
              fi
              az storage blob upload \
                ...
                --account-name "${replace(local.project_full, "-", "")}checkoutsa" \
                --account-key ${data.azurerm_key_vault_secret.web_storage_access_key.value} \
                --file "${path.root}/.terraform/tmp/jwks.json" \
                --overwrite true \
                --name '.well-known/jwks.json'
