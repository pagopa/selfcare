<policies>
    <inbound>
        ...
        <choose>
            <when condition="@{
                var raw = context.Request.Headers.GetValueOrDefault("Content-Length", "0");
                long size;
                return long.TryParse(raw, out size) && size > 10485760;
            }">
                <return-response>
          ...
          "message": "File size exceeds the allowed limit of 10 MiB."
        }</set-body>
                </return-response>
            </when>
        </choose>
